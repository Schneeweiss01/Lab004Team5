...
//Benjamin Schneeweiss, Annabelle Pundaky, Evan Michaelson, Harrison Angellotti
...
//*************************************************************************************************
//CODE REGARDING LIMIT SWITCH
 if(!digitalRead(ciLimitSwitch))  //If Limit Switch Is Hit
 {
 ...
 //*************************************************************************************************
 //BEACON CODE (Visibility and bytes)
...
// Serial.println(iIncomingByte, HEX);        // uncomment to output received character
 ...
 {
    // check to see if elapsed time exceeds allowable timeout then we know we have lost the beacon
...
//*************************************************************************************************    
      ...
      if (btRun)
      {
        ...
        if (state == 0)//Move forward
        {
          CR1_ulMotorTimerNow=millis();
           if( CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <=3750)//start correction
            {
          ...
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 800)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
        ...
        {
          CR1_ulMotorTimerNow=millis();
           if( CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <=5100)//go this distance
            {
            ENC_SetDistance(100, 100);
            ucMotorState = 1;   //forward
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
          ...
          else if(state ==3)//Turn Right
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 780)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
            ENC_SetDistance(-(ci8LeftTurn), ci8LeftTurn);
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
          ...
          if (state == 4)//Move forward
        {
          CR1_ulMotorTimerNow=millis();
           if( CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <=3750)//go this distance
            {
            ENC_SetDistance(100, 100);
            ucMotorState = 1;   //forward
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
   ...
   //   asm volatile("esync; rsr %0,ccount":"=a" (vui32test2)); // @ 240mHz clock each tick is ~4nS 
    ...
    //###############################################################################
    case 3: //Behind the scenes but may be useful
    {
      //move bot X number of odometer ticks
      if(ENC_ISMotorRunning())
      {
        MoveTo(ucMotorState, CR1_ui8LeftWheelSpeed,CR1_ui8RightWheelSpeed);//Useful function to specify direction and speed.
      }
    ...
    //###############################################################################
    case 7://BASED ON IR READING SET LED COLOUR 
    {
